...
int8_t netOpt::assistant2washing(roomMember *assistant, roomMember *washing)
{
                    ...
                    if(a4->variable == 0 && (a4->state == 0 || a4->state == 1))
                    {
                        if(a1->timestamp < a3->timestamp)
                        {
                            if(a2->timestamp > a3->timestamp)
                            {
                                ...
                                {
                                    if(probChange >= -103 - getProbAdjustment(d1, d2, 0.0))
                                    {
                                        probChange = probChange -25 + getProbAdjustment(d1, d2, -2.5);
                                    }
                                    else
                                    {
                                        probChange = -128;
                                        getProbAdjustment(d1, d2, -2.5);
                                    }
                                }
                                else
                                {

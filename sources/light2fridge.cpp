...
int8_t netOpt::light2fridge(roomMember *light, roomMember *fridge)
{
    ...
    uint8_t mac[6];
        ...
        cout << dec << " with probability adjustment of " << getProbAdjustment(d1, ((devRecord *)l1->mems.getHead()->data), 0.0) << endl;
    #endif
    ...
    while(listIteratorA1 != NULL && listIteratorA2 != NULL && listIteratorA3 != NULL)
    {
        ...
        { 
            ...
            {
                if(a3->variable == 1 && a3->timestamp >= a1->timestamp)
                {
                    if(a2->timestamp > a3->timestamp)
                    {
                        if(6 + getProbAdjustment(d1, ((devRecord *)l1->mems.getHead()->data), 0.0) > 0)
                        {
                            if(probChange <= 121 - getProbAdjustment(d1, ((devRecord *)l1->mems.getHead()->data), 0.0))
                            {
                                probChange = probChange + 6 + getProbAdjustment(d1, ((devRecord *)l1->mems.getHead()->data), 0.6);
                            }
                            else
                            {
                                probChange = 127;
                                getProbAdjustment(d1, ((devRecord *)l1->mems.getHead()->data), 0.6);
                            }
                        }
                        else
                        {

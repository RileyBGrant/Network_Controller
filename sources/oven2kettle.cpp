...
int8_t netOpt::oven2kettle(roomMember *oven, roomMember *kettle)
{
    ...
    devRecord *OV1 = (devRecord *)oven->member;
        ...
        cout << dec << " with probability adjustment of " << (int)getProbAdjustment(d1, OV1, 0.0) << endl;
    #endif
                    ...
                    if(a4->variable == 0 && a4->state != a3->state)
                    {
                        if(a1->timestamp <= a3->timestamp)
                        {
                            if(a2->timestamp > a3->timestamp)
                            {
                                if(a3->state == 2)
                                {
                                    if(25 + getProbAdjustment(d1, OV1, 0.0) > 0)
                                    {
                                        if(probChange <= 102 - getProbAdjustment(d1, OV1, 0.0))
                                        {
                                            probChange = probChange + 25 + getProbAdjustment(d1, OV1, 2.5);
                                        }
                                        else
                                        {
                                            probChange = 127;
                                            getProbAdjustment(d1, OV1, 2.5);
                                        }
                                    }
                                    else
                                    {
                ...
                else if(a3->variable == 0 >= 1 && a3->variable <= 6)
                {
                    if(a1->timestamp < a3->timestamp && a2->timestamp > a3->timestamp)
                    {
                        if(5 + getProbAdjustment(d1, OV1, 0.0) > 0)
                        {
                            ...
                            {
                                probChange = probChange + 5 + getProbAdjustment(d1, OV1, 0.5);
                            }
                            else
                            {
                                probChange = 127;
                                getProbAdjustment(d1, OV1, 0.5);
                            }
                        }
                        else
                        {

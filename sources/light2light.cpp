...
int8_t netOpt::light2light(roomMember *m1, roomMember *m2)
{
    ...
    while(listIteratorA1 != NULL && listIteratorA2 != NULL)
    {
        ...
        {
            if(6 + getProbAdjustment(((devRecord *)g1->mems.getHead()->data), ((devRecord *)g2->mems.getHead()->data), 0.0) > 0)
            {
                if(probChange <= 121 - getProbAdjustment(((devRecord *)g1->mems.getHead()->data), ((devRecord *)g2->mems.getHead()->data), 0.0))
                {
                    probChange = probChange + 6 + getProbAdjustment(((devRecord *)g1->mems.getHead()->data), ((devRecord *)g2->mems.getHead()->data), 0.6);
                }
                else
                {
                    probChange = 127;
